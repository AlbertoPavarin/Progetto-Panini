function getBreakByPickup(selected)
{
    ...
    fetch(`https://localhost/Progetto-Panini/food-api/API/order/pickup/getPickupIdBreak.php?PICKUP_ID=${selected.value}`)
    .then((response) => response.json())
    .then((data) => {
        data.forEach(bbreak => {
            fetch(`https://localhost/Progetto-Panini/food-api/API/order/break/getBreak.php?BREAK_ID=${bbreak['break']}`)
            .then((response => response.json()))
            .then(data => {
